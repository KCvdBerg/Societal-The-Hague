...
def RunExperiments(scenario, replications, runtime): # Runs per scenario
    ...
    scenario_bridge_waiting_data = pd.DataFrame(columns=['replication', 'waiting_time', 'bridge_id'])
    ...
    for replication_nr, seed in enumerate(seeds):
        ...
        replications_bridge_data_wait = pd.DataFrame.from_dict(bridge_data_wait, orient='index', columns = ['waiting_time'])
...
scenarios = [[0, 0, 0, 0],[3,5,11,21],[4,8,16,32]]
#
...
for scenario_nr, scenario in enumerate(scenarios):
    scenario_drive_data, scenario_bridge_waiting_data, scenario_bridge_breakdown_chances,  scenario_bridge_truck_counter = RunExperiments(scenario,
                                                                                                          replications,
                                                                                                          run_length)
    scenario_drive_data.to_csv('../experiment/scenario{}.csv'.format(scenario_nr+1), index=False)
    scenario_bridge_waiting_data.to_csv('../experiment/scenario{}_bridges_wait.csv'.format(scenario_nr+1), index=False)
    scenario_bridge_breakdown_chances.to_csv('../experiment/scenario{}_bridges_breakdown.csv'.format(scenario_nr+1),index=False)
    scenario_bridge_truck_counter.to_csv('../experiment/scenario{}_bridges_truck_counter.csv'.format(scenario_nr+1),index=False)